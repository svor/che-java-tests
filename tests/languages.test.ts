...
import { SymbolInformation, Location, Hover } from 'vscode';
...
describe('Che-Java sample tests on Quarkus Project', () => {
    ...
    it('Test Java Document Symbols on basic java file', async () => {
        await vscode.window.showTextDocument(mySampleURI);
        const symbols: SymbolInformation[] = await testservice.languageserver.symbols(extensionID, {
            uri: mySampleURI
        }, new vscode.CancellationTokenSource().token);
    ...
    it('Test Java implementation on basic java file', async () => {
        await vscode.window.showTextDocument(mySampleURI);
        const implementation: Location[] = await testservice.languageserver.implementation(extensionID, {
            uri: mySampleURI
        }, {
                column: 39,
                lineNumber: 14
    ...
    it('Test Java type definition on basic java file', async () => {
        await vscode.window.showTextDocument(mySampleURI);
        const typeDefinition: Location[] = await testservice.languageserver.typeDefinition(extensionID, {
            uri: mySampleURI
        }, {
                column: 35,
                lineNumber: 14
    ...
    it('Test Java hover on basic java file', async () => {
        await vscode.window.showTextDocument(mySampleURI);
        const hover: Hover = await testservice.languageserver.hover(extensionID, {
            uri: mySampleURI
        }, {
                column: 35,
                lineNumber: 14
    ...
    it('Test Java Workspace Symbols on basic java file', async () => {
        const workspaceSymbols: SymbolInformation[] = await testservice.languageserver.workspaceSymbols(extensionID,
            {
                query: "MySample"
            },
            new vscode.CancellationTokenSource().token);
    ...
    it('Test Java references on basic java file', async () => {
        ...
        const references: Location[] = await testservice.languageserver.references(extensionID, {
            uri: myHelloTextURI
        }, {
                column: 16,
                lineNumber: 6
